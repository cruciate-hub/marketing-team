...
| **blog-publisher** | Publish a completed blog article from Google Docs to Webflow live — reads the doc, normalizes it into the shared intermediate, adds internal links, matches a webinar, resizes the master PNG to 3 image sizes and publishes through `webflow-publisher`. Requires `WEBFLOW_API_TOKEN`. | `marketing-team/skills/blog-publisher/SKILL.md` |
| **webflow-publisher** | Publish a finished, compliance-checked draft to any Webflow CMS collection (blog via blog-publisher; glossary once its field slugs are confirmed) — markdown intermediate → Webflow HTML, image resize, `--dry-run` validation, Data API create / `--replace` / `--update`. Does not write content. Requires `WEBFLOW_API_TOKEN`. | `marketing-team/skills/webflow-publisher/SKILL.md` |
| **glossary-content** | Glossary entries for social.plus/glossary — six fixed sections, at least one table, answer-first definition, 500–900 words; rewrites of existing live entries are the common case. Publishes via webflow-publisher once the Glossary field map is confirmed. | `marketing-team/skills/glossary-content/SKILL.md` |
