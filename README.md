...
**Skill logic** (SKILL.md files in `marketing-team/skills/`): A change requires a `version` bump in `marketplace.json` (and the matching `plugin.json`). Teammates with auto-sync on (Step 4 of the brand-kit / marketing-team README) pick it up automatically at next Claude Code startup. Anyone without auto-sync needs to run `/plugin marketplace update cruciate-hub` and `/plugin update <plugin>@cruciate-hub` once.
