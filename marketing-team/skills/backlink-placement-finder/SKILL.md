...
3. **Reference snapshot (2026-06-15 — re-pull, don't trust this list blindly):** the only genuinely-editorial saturated commercial anchors are "customer engagement" (21 refdomains, dofollow, non-spam) and "social media app development" (11); "community features" (11), "social features" (10), "community based marketing" (~11 across casing variants) and "User-generated content" (8) follow. Note "greater user engagement and loyalty" (24 refdomains, all dofollow) is now flagged `is_spam` — it is a syndication/scrape footprint of one stat phrase, NOT output of this skill, so don't read it as exact-match over-use you can fix here. The profile head is otherwise dominated by branded/URL anchors (healthy) and by PBN/Telegram link-service spam (a disavow concern that lives outside this skill). Fresh (zero presence) still includes the whole retention family, the app-engagement family, app stickiness, activity/news feed, app/community monetization, in-app chat/messaging, real-time messaging, social commerce, live streaming, community platform, in-app community, brand advocate.
...
1. **Identify Phase 1 candidates.** Three candidate types qualify. Treat B and C exactly like A during discovery. Under-flagging B and C is the most common reason this skill leaves strong placements on the table; over-flagging is recoverable in the Step 3.5 decision gate, under-flagging is not.
   ...
   **Type B: Creative anchor match.** A 2-6 word phrase appears verbatim in the partner's body text, is NOT literally in `references/anchors.md`, and is semantically equivalent to a listed anchor. Example: "customer retention" maps to the user-retention anchor family. See `references/creative-anchor-patterns.md` for the full list of confirmed mappings; append new ones there as you discover them.
   ...
   **One carve-out lowers a Type A ceiling: saturated commercial head terms.** A Type A literal match whose phrase is a saturated commercial keyword — already at ≥10 refdomains in the live social.plus profile (see the Anchor Diversity Check; "customer engagement" and "social media app development" are today's examples) — does NOT reach ⭐⭐⭐. Cap it at ⭐⭐ and prefer a fresher co-located variant from the same sentence. The Anchor Diversity Check is authoritative over the fit score: when they disagree, the Diversity Check wins. This is a control on our own over-optimization, not a discovery filter — keep flagging the head term during discovery, just don't headline it in the email.
...
6. **Score the fit:**
   - ⭐⭐⭐ **Perfect** — Type A literal anchor verbatim in body paragraph AND partner article traffic ≥ 50/month AND target UR ≥ 10 AND the anchor is NOT a saturated commercial head term (≥10 refdomains in the live profile — see Anchor Diversity Check; a saturated head term caps at ⭐⭐ and should yield to a fresher co-located variant). Type B and Type C never reach this tier, even when traffic and UR clear the thresholds.
