...
0. **Semantic-territory scan (run before keyword matching).** Read each body paragraph end-to-end. For every paragraph, write down 3-5 candidate phrases that could semantically map to ANY social.plus content territory — meaning either an anchor family in `references/anchors.md` OR a topic represented in the blog/glossary inventory (`website/pages-blog.json`, `website/pages-glossary.json` — match against `metaTitle`, `metaDescription`, or `content` heading hierarchy). Be deliberately generous here: cast wider than feels necessary. The point of this pass is to surface phrases that slug-scans and `anchors.md` literal-substring lookups miss — the highest-value creative wins live there. Worked example: "social sellers" in the Pineable AI-photo-tools article (2026-06-19) mapped cleanly to `/blog/from-viewers-to-belonging-why-community-is-the-engine-of-live-commerce`, but appeared in neither `anchors.md` nor the pattern cache. A territory scan surfaces it; a literal-list scan does not. Then validate each candidate from this list against the Type A / B / C criteria in step 1 below; candidates that fail any guardrail drop out, but the scan ensures you considered them in the first place.
