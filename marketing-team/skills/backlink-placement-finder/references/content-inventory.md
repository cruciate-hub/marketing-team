...
**Where candidates come from.** In editorial mode (the default at this stage of the funnel — see SKILL.md Phase 1 step 0), candidates are generated by reading the survivor articles end-to-end and surfacing 5-10 angles per article BEFORE consulting any list. The list-routes below are NOT lookup targets — they are constraints applied to editorially-generated candidates to decide whether each one is valid. In the long-tail fallback (>5 candidate articles per partner), candidates come from the semantic-territory scan instead; either way, the validation routes are the same.
