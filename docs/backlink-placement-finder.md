...
5. **Match against anchors & inventory** — Two-phase approach. Phase 1 starts with a **semantic-territory scan**: read each body paragraph end-to-end and list 3-5 candidate phrases that could map to ANY anchor family in `references/anchors.md` OR ANY topic in the live blog/glossary inventory (`metaTitle` / `metaDescription` / `content` headings). Then validate each candidate against the Type A / B / C criteria. This step is what surfaces creative anchors that slug-scans and literal-list lookups miss — e.g., the Pineable "social sellers" placement maps to a real blog topic on live commerce despite not appearing in `anchors.md`.
