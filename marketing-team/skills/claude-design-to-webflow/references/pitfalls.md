...
### 11. Freeform-code setters REPLACE the entire block — read-then-modify-then-write
The whole custom-code surface is API read/write, and the write side is destructive if you skip the read:
- **Site & Page freeform Head/Footer:** `data_scripts_tool` → `get_site_freeform_code` / `set_site_freeform_code`, `get_page_freeform_code` / `set_page_freeform_code` (location `head` | `footer`). The `set_*` calls **replace the whole block — they do not append**. A careless `set_site_freeform_code` wipes existing schema, analytics, and reveal systems in one call. Always `get_*` first, merge your change into the current block locally, then write the merged block back; snapshot the original to a local file before any large rewrite and verify the untouched bytes survived (→ Pitfall 33 for the byte-for-byte ritual).
- **HTML Embeds (the `<style>`/`<script>` blocks inside components or pages):** they're `HtmlEmbed` elements with the same replace semantics. Read/write their code with `data_element_settings_tool` → `get_settings` / `set_settings`, key **`code`** (`static_text.value` = the full `<style>…</style>` or `<script>…</script>` string). Find the embed element first: `data_element_tool query_elements` with `element_filter.type: "HtmlEmbed"` (scope to a component with `scope_component_id`).
- **Registered scripts** (App-added) are a separate surface: `register_inline_script` / `add_site_script` / `add_page_script` / `get_registered_scripts` — read Pitfall 43 before defaulting to them.
...
**Detect (fully headless):** (1) read the live variable set — `data_variable_tool get_variables { queries:[{}] }` (the `queries` array is required; `pageId` is required top-level) — collect the valid `variable-…` IDs. (2) `data_style_tool get_styles { query:"all", include_properties:true }` — this is large and **persists to a file**; parse with Python/jq, don't inline it. Any property whose value is `{"id":"variable-…"}` where the ID is **not** in the live set is dangling. Group by the missing ID; infer intent from usage (serif/display font-sizes → the serif font var; small body/UI/button sizes → the sans).
...
**Two more ways this check lies to you:** (a) the page-optimised sheet's path includes a `/css/` segment you can easily drop when hand-constructing the URL instead of reading it from the page (`.../<siteId>/css/social-plus.webflow.<pageId>.<hash>.opt.min.css`) — omitting it 404s **silently** and a `grep` on the empty/error body returns zero matches exactly like "styles never compiled," which can burn several publish-and-debug cycles chasing a bug that never existed. Always extract the href from the fetched page HTML, never reconstruct it, and sanity-check the fetched file's byte size before concluding anything from a zero-match grep. (b) minified CSS **rewrites color literals** — `rgba(46,230,205,.75)` ships as `#2ee6cdbf` — so grepping for the rgba/hex string you wrote returns zero even when the rule is live; grep for a distinctive px value or the class name instead. And use `grep -o '…' | wc -l` for real occurrence counts, not `grep -c` — minified CSS is one giant line, so `-c` maxes at 1 regardless of how many times the pattern actually appears.
...
**Prevention is unreliable:** a rule in the `css` param usually saves the class but NOT always — in the same build, `vise-wf-desc` spans lost their class *despite* a css-param rule (second span inside a styled span; the sibling `vise-wf-title` survived). So treat verification as mandatory: after every whtml call that carries classed inline spans, run a live-DOM or `children_depth:-1` check for style-less spans and re-apply.
...
## Part 9 — freeform-code & style-write API traps (surfaced across a multi-week bento-grid rebuild)

### 49. `set_page_freeform_code`/`set_site_freeform_code` reject ANY `<script>` tag with HTTP 406 — even content already stored there
**Symptom:** you `get_page_freeform_code`, see an existing `<script>…</script>` block in the returned content, change one unrelated line, write it back — **406**. Stripping the script and resubmitting CSS-only content succeeds instantly. Re-testing byte-identical, untouched pre-existing content (isolated down to a single `<script>` tag with a trivial IIFE body) **also 406s**, on both `head` and `footer` locations. Angle brackets elsewhere (CSS `@property --x{syntax:'<percentage>'}`) are completely fine — it is specifically `<script>` tags that the write path rejects, even though the *read* path returns them without complaint.
**Implication:** whatever script content IS currently stored in a page's freeform block was set through the Designer's own custom-code UI (which doesn't run this validation), not through this API — and once you need to touch that block for ANY reason via the API, you cannot round-trip the script back in.
**The documented fallback can also dead-end:** `register_inline_script` + `add_page_script`/`add_site_script` is the sanctioned alternative (Pitfall 43) — but on some sites/pages BOTH `add_page_script` and `add_site_script` return **404 "Custom code block not found"** for a script that registered successfully, with no working alternative left at all via the API.
**Recipe:** before writing to a freeform block, `get_*_freeform_code` first and scan for `<script>`. If present: split it out, write back only the script-free HTML/CSS via the API, then try `register_inline_script` → `add_page_script`/`add_site_script` for the script. If that 404s too, stop calling APIs and hand the user the exact `<script>…</script>` text with the literal instruction to paste it into the Designer's Page Settings (or Site Settings) → Custom Code panel — don't keep guessing at routes. Small trap along the way: `register_inline_script`'s `display_name` is schema-validated to **alphanumeric characters only** (no hyphens, no spaces) — `"hc-hero-cloak"` 400s, `"hcHeroCloak"` doesn't.

### 50. Freeform code also 406s on external resource references
**Symptom:** a `<link rel="stylesheet" href="…">` or `@import url(…)` inside `set_page_freeform_code`/`set_site_freeform_code` content returns 406, same failure class as Pitfall 49 — a plain `<style>` block with no external reference is fine.
**Fix:** put the external `<link>`/`@import` in an **HtmlEmbed** element instead (`data_element_settings_tool set_settings` key `code`) — embeds allow external resource references that raw freeform code rejects.

### 51. An active Designer session can lock parts of the write API — don't chase a phantom bug
**Symptom:** while the user has the Designer open on the same page, `update_style` can throw an MPS "[Conflict] style block store" error, `set_style` can fail "styles not found" on classes that visibly exist, and `set_page_freeform_code` can 406 on content that isn't touching Pitfall 49/50 at all. Reads keep working throughout.
**Fix:** this is a session-lock, not a data problem — wait for the Designer tab to close (or ask the user to close it) and retry unchanged. Don't start rewriting styles or freeform content to work around it; you'll ship a style default whose paired script/class change never actually landed.

### 52. Newly-created classes are invisible to `set_style` in a multi-class array for several minutes
**Symptom:** `set_style {style_names:["new-class"]}` alone works instantly after `create_style`; `set_style {style_names:["existing-class","new-class"]}` fails "styles not found" for the same class, for several minutes — even though `query_styles` already returns it. Waiting 5+ minutes in-session did not reliably resolve it.
**Fix:** don't create-then-combine in two calls. Use `data_whtml_builder` with the full class list in the `html` and matching rules in the `css` param — it creates the element, the classes, and the class *application* in one atomic pass, bypassing the lookup that's stale. Design new decorative compositions as self-contained whtml inserts rather than `create_style` → `set_style`.

### 53. `set_style` REPLACES the element's ENTIRE class list — never use it to "just test one class"
**Symptom:** a quick debugging call, `set_style {style_names:["test-class"]}`, on an element that already carried `["card","card-w2"]` — silently strips both original classes. The element loses `position:relative`, `overflow`, its grid-column-span, everything `card`/`card-w2` provided; its absolutely-positioned children then re-anchor to the nearest OTHER positioned ancestor and render on top of a completely different section. This shipped to a live published page before being caught (in the Designer AND on staging simultaneously).
**Fix:** `set_style` is a full replacement, not a toggle/add — always pass the element's complete intended class list, never a single class "just to check something." After any `set_style` experiment, immediately re-query the element's `styleNames` to confirm what actually landed, and audit sibling elements for collateral damage if the call touched a shared layout.

### 54. whtml's `css` param rejects pseudo-elements — but `update_style` on an existing class accepts them fine
**Symptom:** `data_whtml_builder` with a `css` param containing a `::before`/`::after` rule fails with an explicit parser error refusing the `::` selector.
**Fix:** build the overlay/decoration as a real nested child `<div>` in the `html` instead of a pseudo-element. If you need a pseudo-element specifically, apply it AFTER the class exists via `data_style_tool update_style {…, pseudo:"before"}` — that path has no such restriction.

---

## Part 10 — CSS rendering traps: blend modes, color rewriting, breakpoint sizing & headless-Chrome limits

### 55. `mix-blend-mode` on a nested element can bleed through its own container and blend with the WHOLE PAGE
**Symptom:** a `mix-blend-mode:soft-light` grain/texture layer scoped to one card, with `isolation:isolate` on the card itself, still visibly blends against content far outside the card — shipped as a page-wide washed-out gray film on a live published page. `isolation:isolate` on the ancestor did **not** reliably contain it in this rendering context.
**Fix:** don't use blend modes for texture/grain overlays in Webflow at all. For plain texture, a low-opacity layer with no blend mode is enough. For breaking up gradient **banding** specifically (the situation blend modes are usually reached for), two `feTurbulence` SVG data-URI noise layers on one element — a white-speckle layer (alpha ≈.42) stacked with a black-speckle layer (alpha ≈.62), combined at ~.3 layer opacity — reads as a frosted, grain-broken surface with zero blend mode and neutral luminance (blacks stay black, unlike a single white-only speckle layer which visibly lightens dark regions).

### 56. Runtime-only combo classes get silently PURGED as "unused" on the next Designer save/publish
**Symptom:** a combo class that JS only ever *adds at runtime* (never present on any element inside the Designer itself) works fine right up until the next time the Designer autosaves or the user publishes from the Designer — then it's gone, along with whatever hover/active state it drove. Reads as "the styling broke on its own."
**Cause:** Webflow's cleanup treats "no element in the Designer currently carries this class" as unused and purges the Style record, regardless of whether JS applies it at runtime.
**Fix:** permanently apply the combo to ONE real element in the Designer — ideally an element where that state is ALSO the correct default/no-JS-yet state, so the assignment is doubly justified. That makes the class genuinely "used," visible in the canvas, and survives cleanup; the runtime script just moves the class around from there. After any Designer session on a page with runtime-toggled classes, re-grep the published CSS for the combo selector to confirm it's still there.

### 57. Headless Chrome on macOS silently refuses to render below ~485 CSS px
**Symptom:** `--window-size=390,H --screenshot=out.png` produces a 390-px-wide PNG that LOOKS exactly like a genuine mobile-overflow bug — nav items and body text sliced off at the right edge — but it's actually a true ~485px-wide render that got cropped to the requested 390, not real content overflow.
**Fix:** don't diagnose small-viewport (< ~485px) issues from headless-Chrome screenshots at all. Measure the real thing instead: inject a probe script that writes `clientWidth`/`scrollWidth` and any offending element rects into the DOM, then read it back with `--dump-dom` (no CDP/websocket dependency needed); or verify on a real device / simulator.

### 58. Put desktop-only, wide/overlapping layouts on `large` (≥1280px), never on base
**Symptom:** a wide multi-item rail or overlapping-card layout sized for a 1280px container, set on the base breakpoint, overflows every viewport from 992px to 1279px — base applies 0–1279 (→ Breakpoints table above), so "desktop" values there leak into the whole tablet-and-up range below 1280.
**Fix:** base = the safe/narrow-container grid, `large` = the expressive wide-desktop treatment, `medium`/`small`/`tiny` = further downsizing from there. Applies to combo-class transforms too — a rotation/transform set on a combo's BASE beats a `transform:none` set on the parent class at a narrower breakpoint, because selector specificity outranks the media query.

---

## Part 11 — real-artwork compositing, Figma-sourced designs & verification methodology (added after a from-scratch bento-grid design port)

### 59. Bake the design tool's own export as the background, under live UI, instead of hand-recreating it in CSS
**Context:** porting a visually rich section (glows, gradients, texture, photography) where the goal is genuinely pixel-identical to a design reference, not merely close. Hand-built CSS gradients/textures have a hard ceiling on organic, hand-made, or dithered artwork no matter how carefully colors and geometry are sampled from the reference (→ Pitfall 60) — two full iteration rounds of "more precise" gradient forensics still read as visibly synthetic.
**Recipe that actually reaches 1:1:** export each section/card at 2x from the design tool **at its own native design coordinates** — don't rescale or reposition it. Set it as the live element's `background-image` at `background-size: auto <designHeightPx>px` (locks the export's native aspect ratio) — **never `background-size:100% 100%`**, which silently skews/compresses the baked art (most visible as compressed or "kerned-together" baked TEXT) the instant the live container's rendered size drifts even slightly from the export's original design size — which it will, after any grid restructure, breakpoint reflow, or content-driven resize. Then let the real, live, interactive UI render on top at the same coordinates the export was measured from — since both were built from the same design, everything lines up within a pixel with no manual alignment needed.
**Three traps inside that recipe:**
1. **Don't apply an alignment "correction."** A template-match score suggesting a small pixel shift is usually wrong; an unshifted (0,0) placement is usually right, and baked TEXT is the tell — text ghosting/doubling against the live DOM at a shifted offset is what proves zero-offset was correct, since text is the only element sharp/high-contrast enough to disconfirm a plausible-looking shift.
2. **Baked multi-line text still visibly double-images against live DOM text at the same position**, even at the correct offset, because the design tool wraps and kerns text differently than the browser does. Fix by setting the LIVE text's `color` to `transparent` — the pixels come from the baked art, the real text stays in the DOM for accessibility and SEO, and the double-image disappears.
3. **A live interactive element meant to sit over its own glyph baked into the art** (e.g., a button icon that's both baked into the export AND rendered live on top so it stays clickable) must be anchored from the SAME edge the export was authored from — almost always design-left, via an explicit `left:<designPx>px; right:auto` override — never the CSS-conventional opposite edge (`right:1.5rem`), which re-measures from the container's actual right edge on every render and visibly separates from its baked twin the moment the container's rendered width isn't exactly the design width.

### 60. "1:1 to a design reference" is easy to falsely certify — point-sampling passes while a human still sees a fake
**Symptom:** coarse pixel-color sampling and cropped-screenshot comparisons report a match, get reported as "done," and the result still visibly reads as an approximation to a human reviewer.
**Cause:** the tells a human catches instantly — gradient banding, plastic-smooth falloff, missing grain/texture — are exactly what coarse pixel sampling is blind to; a handful of sample points can match while the overall gestalt is wrong.
**Fix:** verify with precisely-ALIGNED side-by-side crops — real DOM `getBoundingClientRect()` rects on both the live page and the reference, at the same scale — not eyeballed regions or eyeballed screenshots. And treat two rounds of "more precise" gradient iteration without closing the visible gap as a signal to stop iterating CSS and switch to Pitfall 59's baked-export approach, rather than attempting a third, still-doomed round of hand-tuning.

### 61. Figma-sourced designs: `use_figma` follows whatever file is OPEN, not the `fileKey` argument; `export_video` needs a top-level frame
**Symptom A:** `use_figma {fileKey: X, code: "...node lookup..."}` returns "node not found" even for a node you can see exists in file X.
**Cause:** the plugin tool runs inside whatever file the user's Figma **desktop app** currently has open — a mismatch with the `fileKey` argument doesn't error, it just silently operates on the wrong (possibly empty) file.
**Fix:** probe before trusting a lookup failure — `figma.root.name`, `figma.currentPage.children.length` — to confirm you're actually in the intended file before concluding a node doesn't exist.
**Symptom B:** `export_video` on a nested/sub-clip node errors "Export root must be a top-level frame."
**Fix:** export the whole top-level frame as one video, then crop the specific element out with ffmpeg using that element's x/y/width/height from `get_metadata`.

### 62. Don't hand-extract "grain" from a flattened design export — it's a trap, and it's usually unnecessary
**Symptom:** a high-pass-filtered "grain" tile cropped from a reference export, then contrast-boosted and mirror-tiled for coverage, ships as glaring repeated artifacts (verified incident: bright outline rings repeating across every card) instead of subtle texture.
**Cause:** a small high-pass crop is likely to pick up structure that isn't grain at all — a UI mockup's rounded corner, an edge, a seam — and tiling amplifies that structure across the whole surface. Separately: genuinely clean grain regions in a real export high-pass to near-nothing (alpha ≤16/255) — what usually reads as "grainy" in a reference is color dithering at gradient edges, not an applied texture layer.
**Fix:** procedural SVG `feTurbulence` noise at low opacity (→ Pitfall 55) is visually equivalent to real extracted grain and carries none of this risk. Whichever source you use, render the candidate tile at its intended final opacity, at actual card size, and look at it before applying it anywhere — the failure mode above is invisible at 100% opacity in an image viewer and only shows up composited at low opacity on the real surface.

### 63. A session's scratch/temp directory can be wiped mid-session — copy one-off user inputs out immediately
**Symptom:** a user-provided reference export or screenshot, needed later in the same session, is simply gone from the scratchpad — no error, just absent.
**Fix:** the moment the user hands over a design reference, export, or any input meant to be used more than once, copy it to a durable path right away — don't leave the only copy sitting in scratch. This is agent hygiene, not a Webflow-specific bug, but it bit a real migration hard enough to note here: assets already uploaded to the Webflow CDN survive independently and don't need this treatment.
